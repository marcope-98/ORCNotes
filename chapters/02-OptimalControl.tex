...
\begin{theorem}
If $\mathcal{X}_f$ is control invariant then the MPC is \side{persistently feasible}
\end{theorem}
...
\begin{proof}
\begin{enumerate}
\item Given $x_n \in \mathcal{X}_f = \mathcal{X}_n$ define \side{backward reachable sets} recursively:
